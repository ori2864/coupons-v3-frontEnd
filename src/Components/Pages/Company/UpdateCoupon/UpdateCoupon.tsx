...
import { addCouponAction, updateCouponAction } from "../../../Redux/companyReducer";
import { updateGuestCouponAction } from "../../../Redux/guestReducer";
...
export function UpdateCoupon(): JSX.Element {
    ...
    useEffect(()=>{
        if(!checkAuth("COMPANY")){
            ...
            checkData();
            systemStore.dispatch(titleAction("Update Coupon"))
        }
        ...
        const makeCoupon: SubmitHandler<Coupon> = (data) => {
            ...
            data.companyID = systemStore.getState().auth.workId;
            console.log(data)
            axiosJWT.put("http://localhost:8080/api/company/update/coupon",data).then(res=>{
                    systemStore.dispatch(updateCouponAction(data));
                    systemStore.dispatch(updateGuestCouponAction(data));
            ...
            .catch(err => {
                    notify.error("problem occurred with update coupon");
    ...
    return (
        <div className="UpdateCoupon" style={{ display: "flex", justifyContent: "center", alignItems: "center", }}>
			
            <div className="Box" style={{ width: "50%", }}>
			<Typography variant="h4" style={{textAlign:"center"}} className="HeadLine">Update Coupon</Typography>
                <hr /><br/>
                <form onSubmit={handleSubmit(makeCoupon)}>
                    <TextField required type="number" label="Coupon Id" fullWidth {...register("id", { required: true })} />
                    <br /><br />
                    ...
                    {errors.title?.type == "maxLength" && <><br/><span style={{ color: "red" }}>Title is too long. Max length: 15</span></>}
                    <br /><br />
